...
harness.state.workspace = {
  ...
  calendar_events: [
    ...
    { id: "event-2", project_id: "", event_type: "休假", title: "Escaped <notice>", date: today, all_day: true, time: "", end_time: "" },
  ],
...
assert(harness.calendarImportantType({ event_type: "要約面試" }) === "邀約面試", "The previous interview-invitation label should migrate without losing the event");
assert(harness.calendarImportantColor({ event_type: "休假" }).color === "#b9485c", "Vacation should use red");
